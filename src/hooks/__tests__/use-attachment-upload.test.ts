...
describe("useAttachmentUpload", () => {
  describe("addAttachments", () => {
    ...
    it("creates blob URLs via URL.createObjectURL", () => {
      ...
      expect(result.current.pendingAttachments[0].blobUrl).toBe("blob:mock-1");
    });

    it("opens XHR to POST /api/internal/assistant/attachments", () => {
      const { result } = renderHook(() => useAttachmentUpload());
      ...
      expect(xhr.open).toHaveBeenCalledWith(
        "POST",
        "/api/internal/assistant/attachments"
      );
    });
    ...
    it("assigns unique IDs to each image", () => {
      ...
      act(() => {
        result.current.addAttachments([makeFile("a.png"), makeFile("b.png")]);
      });

      const ids = result.current.pendingAttachments.map((i: PendingAttachment) => i.id);
  ...
  describe("progress tracking", () => {
    it("updates progress percentage on XHR upload.onprogress", () => {
      ...
      expect(result.current.pendingAttachments[0].progress).toBe(50);
    });
  ...
  describe("upload success", () => {
    it("sets status 'done', progress 100, and filename on 200 with valid JSON", () => {
      ...
      const img = result.current.pendingAttachments[0];
  ...
  describe("removeAttachment", () => {
    it("aborts the XHR and revokes the blob URL", () => {
      ...
      const blobUrl = result.current.pendingAttachments[0].blobUrl;
    ...
    it("removes the image from pendingAttachments", () => {
      const { result } = renderHook(() => useAttachmentUpload());
  ...
  describe("clearAll", () => {
    it("aborts all XHRs, revokes all blob URLs, and empties pendingAttachments", () => {
      const { result } = renderHook(() => useAttachmentUpload());
      ...
      const blobUrls = result.current.pendingAttachments.map((i: PendingAttachment) => i.blobUrl);
  ...
  describe("hasUploading", () => {
    ...
    it("is false when pendingAttachments is empty", () => {
      const { result } = renderHook(() => useAttachmentUpload());
  ...
  describe("unmount cleanup", () => {
    it("revokes all blob URLs and aborts all in-flight XHRs on unmount", () => {
      const { result, unmount } = renderHook(() => useAttachmentUpload());
    ...
    it("does not throw when unmounted with no images", () => {
      const { unmount } = renderHook(() => useAttachmentUpload());
