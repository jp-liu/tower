...
export const taskTools = {
  ...
  create_task: {
    description:
      ...
      "Labels: call `list_labels` FIRST. If the workspace has any, picking the single best-fitting one for `labelIds` is REQUIRED, same standing as the description format — judge from what the task actually does. A label may carry a branch prefix that names the task's worktree branch, so a fitting label left out silently falls back to the default prefix. Extra labels are allowed when they genuinely apply. If the workspace has no labels at all, omit `labelIds` — do not invent labels. " +
