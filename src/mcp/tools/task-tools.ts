...
export const taskTools = {
  ...
  create_task: {
    description:
      ...
      "When the user also asks to enable unattended goal mode, call `set_goal_mode({ taskId: <created id>, on: true })` immediately after this tool returns and before ending the turn; task auto-start does not enable goal mode. " +
      "When the response includes `handoff.mode=callback`, finish any explicitly requested `set_goal_mode` call first, then end the current turn and wait for Tower to inject the child's completion message. Do not sleep, poll execution status/output, or inspect the child worktree unless the user explicitly asks for progress or you are recovering from a suspected missed callback. " +
