...
function installOpenClawProfile(input: {
  ...
  try {
    ...
    writeEnvFile(path.join(workspaceDir, "gateway.env.example"), input.env);
    writeMarker(path.join(workspaceDir, ".tower-agent.json"), {
      gateway: "openclaw",
...
async function installHermesProfile(input: {
  ...
  try {
    ...
    writeEnvFile(path.join(profileDir, "gateway.env.example"), input.env);
