...
export const CONFIG_DEFAULTS: Record<string, ConfigEntry> = {
  ...
  "task.workbenchDirective": {
    defaultValue: [
      ...
      "- **After `create_task` confirms an auto-started execution, end the current turn immediately and wait for Tower's child-completion callback.** Do not sleep, poll `get_task_execution_status`, read the child terminal, inspect its worktree, or otherwise babysit it. Tower injects a review message into this terminal when the child finishes a round. Status checks are only for an explicit user request or recovery after a suspected missed callback.",
      "",
      "## Review",
