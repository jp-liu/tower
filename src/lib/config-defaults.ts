...
export const CONFIG_DEFAULTS: Record<string, ConfigEntry> = {
  ...
  "task.systemDirective": {
    defaultValue: [
      ...
      "- Dangerous / irreversible operations (dropping a database, deleting a large directory, `rm -rf`, `drop table`, force-push, external release) → always get consent via `ask_human` first before executing, even when the terminal has already granted permissions.",
