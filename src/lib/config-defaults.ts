...
export const CONFIG_DEFAULTS: Record<string, ConfigEntry> = {
  ...
  "task.systemDirective": {
    defaultValue: [
      ...
      "- Use `tower-bridge` for every external side effect. Explicitly named human/group recipients use its `human.message.send / explicit` path with `push_to_human`; unattended OWNER messages use `human.message.send / owner_home` with one bounded CapabilityRequest; computer, browser, SaaS, document, spreadsheet, and other capabilities use the same bridge skill. Never hard-code an operator agent. For a sibling Tower task, stay inside Tower: use `resume_task_execution` and `send_task_terminal_input`.",
