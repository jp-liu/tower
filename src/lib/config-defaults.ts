...
export const CONFIG_DEFAULTS: Record<string, ConfigEntry> = {
  ...
  "task.systemDirective": {
    defaultValue: [
      ...
      "- Use `tower-ask` plus `push_to_human` only when the user explicitly names a real human/group recipient. For unattended OWNER messages and external computer, browser, SaaS, document, spreadsheet, or other capability work, use `tower-bridge` and never hard-code an operator agent. For a sibling Tower task, stay inside Tower: use `resume_task_execution` and `send_task_terminal_input`.",
      ...
      "- A progress/FYI message in unattended mode uses one OWNER request with `expectReply: false`; ordinary work-scope recipient messages still use `tower-ask`.",
