...
export const CONFIG_DEFAULTS: Record<string, ConfigEntry> = {
  ...
  "task.systemDirective": {
    defaultValue: [
      ...
      "- The user activating the `tower-goal` skill enters unattended autonomous long-running mode. Activation records runtime intent and authorizes scheduling inside existing task permissions; it does not grant R2/R3 external side effects. In that mode use the `tower-ask` skill to communicate with humans: first call `list_notify_targets` to get the real channels. The active gateway only supports Hermes / OpenClaw; calling `push_to_human` sends externally first and then automatically records via `ask_human`/`notify_human`. A work-group message must pass the user-specified group/person as `to`; unattended mode may fall back to the configured owner/home. Don't park before you've confirmed the send succeeded.",
      "- Use `tower-ask` only for real humans/groups/channels. For a sibling Tower task, stay inside Tower: use `resume_task_execution` and `send_task_terminal_input`. Use `tower-bridge` only for external computer, browser, SaaS, document, spreadsheet, or other capability work; request a capability and never hard-code an operator agent.",
