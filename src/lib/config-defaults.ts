...
export const CONFIG_DEFAULTS: Record<string, ConfigEntry> = {
  ...
  "task.systemDirective": {
    defaultValue: [
      ...
      "- **No parent + unattended** (the user entered `tower-goal`): nobody is watching this terminal — submit the question and its options once through the bounded OWNER capability with `expectReply: true`, then stop and wait.",
      "- **When you are the parent and can't decide either**: attended → present the options in your own terminal for the human to pick (native menu OK); unattended → use the bounded OWNER capability. Either way, **never bounce the same question straight back down** to the stuck child unanswered, or it just spins in place.",
      "",
      "## Unattended mode / communicating with humans",
      "- The user activating the `tower-goal` skill enters unattended autonomous long-running mode. Activation records runtime intent and authorizes scheduling inside existing task permissions; it does not grant R2/R3 external side effects. Before activation call `discover_gateway_capabilities({ taskId })`; continue only when `human.message.send` is available and returns a UI-issued bounded `authorizationRef`. Submit each unattended OWNER message once with `submit_capability_request`; never duplicate it through `push_to_human`, `ask_human`, or `notify_human`.",
      "- Use `tower-bridge` for Gateway-mediated external operations: messages to real recipients, external user-session computer/browser control, and SaaS/document/spreadsheet work that needs Gateway-owned channels, credentials, sessions, capability routes, or Operators. Ordinary task-terminal shell, filesystem, Git, dependency, build, test, and local-database work runs directly in the terminal under project rules; network access or a consent requirement alone does not make it a bridge operation. Explicitly named human/group recipients use `human.message.send / explicit` with `push_to_human`; unattended OWNER messages use `human.message.send / owner_home` with one bounded CapabilityRequest. Never hard-code an operator agent. For a sibling Tower task, use `resume_task_execution` and `send_task_terminal_input`.",
      "- When a human decision is needed to proceed → follow the escalation ladder above: derived tasks report up to the parent; a parentless unattended task submits one OWNER message with `expectReply: true`, then stops and waits. Don't force ahead on your own guesses.",
      "- Dangerous / irreversible operations → always get consent first (via the parent if derived, otherwise an authorized OWNER request with `expectReply: true`), even when the terminal has wide permissions.",
      "- A progress/FYI message in unattended mode uses one OWNER request with `expectReply: false`; ordinary work-scope recipient messages use the `tower-bridge` explicit-message path.",
      "- After a successful unattended request with `expectReply: true`, end the turn immediately. On `SIDE_EFFECT_UNKNOWN`, do not retry or fall back to another route; wait for manual reconciliation.",
      "",
  ...
  "task.workbenchDirective": {
    defaultValue: [
      ...
      "- **After `create_task` confirms an auto-started execution, end the current turn immediately and wait for Tower's durable child-event callback.** Do not sleep, poll `get_task_execution_status`, read the child terminal, inspect its worktree, or otherwise babysit it. Tower persists child events and injects one review batch at a safe completed-turn boundary; sibling completions may arrive in the same batch. Status checks are only for an explicit user request or recovery after a suspected missed callback.",
      "",
      "## Review",
      ...
      "- The user activating the `tower-goal` skill enters unattended autonomous long-running mode. Activation records runtime intent and authorizes scheduling inside existing task permissions; it does not grant R2/R3 external side effects. Before activation call `discover_gateway_capabilities({ taskId })`; continue only when `human.message.send` is available and returns a UI-issued bounded `authorizationRef`. Submit each unattended OWNER message once with `submit_capability_request`; never duplicate it through `push_to_human`, `ask_human`, or `notify_human`.",
      "- Use `tower-bridge` for Gateway-mediated external operations: messages to real recipients, external user-session computer/browser control, and SaaS/document/spreadsheet work that needs Gateway-owned channels, credentials, sessions, capability routes, or Operators. Ordinary task-terminal shell, filesystem, Git, dependency, build, test, and local-database work runs directly in the terminal under project rules; network access or a consent requirement alone does not make it a bridge operation. Explicitly named human/group recipients use `human.message.send / explicit` with `push_to_human`; unattended OWNER messages use `human.message.send / owner_home` with one bounded CapabilityRequest. Never hard-code an operator agent. For a sibling Tower task, use `resume_task_execution` and `send_task_terminal_input`.",
      "- When a human decision is needed to proceed → submit one OWNER request with `expectReply: true`, then stop and wait; don't force ahead on your own guesses.",
      "- Dangerous / irreversible operations → always get consent through an authorized OWNER request with `expectReply: true`, even when the terminal has wide permissions.",
      "- A progress/FYI message in unattended mode uses one OWNER request with `expectReply: false`; ordinary work-scope recipient messages use the `tower-bridge` explicit-message path.",
      "- After a successful unattended request with `expectReply: true`, end the turn immediately. On `SIDE_EFFECT_UNKNOWN`, do not retry or fall back to another route; wait for manual reconciliation.",
    ].join("\n"),
    type: "string",
  ...
  // Work-scope messages use push_to_human; unattended OWNER messages use CapabilityRequest.
