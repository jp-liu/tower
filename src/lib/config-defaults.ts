...
export const CONFIG_DEFAULTS: Record<string, ConfigEntry> = {
  ...
  "task.systemDirective": {
    defaultValue: [
      ...
      "",
      "## 无人值守 / 与人沟通",
      "- 用户主动激活 `tower-goal` 技能即进入无人值守自主长跑（激活即赋权）；此时用两个 MCP 工具与人沟通：`ask_human`（阻塞：**记录**问题并挂起任务——本回合到此为止，人回复后自动唤醒、带答案续跑）和 `notify_human`（非阻塞：**记录**一条进度/FYI 后立即继续，不必等回复、不挂起）。**注意：这两个工具只在 Tower 内记录（/harness 面板可见），本身不把消息发给人**；要真正通知到人，须先用 `tower-ask` 技能（调 `list_notify_targets` 拿到填好真实渠道的发送指令）经平台渠道把消息发出去，再调这两个工具留档。",
  ...
  // 经 tower-ask 技能（list_notify_targets 组装发送指令）用平台 MCP（飞书/微信/…）完成。故这里不再有平台凭据/渠道绑定配置。
