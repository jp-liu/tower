...
export const CONFIG_DEFAULTS: Record<string, ConfigEntry> = {
  ...
  "task.systemDirective": {
    defaultValue: [
      ...
      "",
      "## When stuck / a decision needs someone above you (escalation ladder)",
      "- **Never wait on the CLI's native `AskUserQuestion` prompt, and never sit idle on a plan / option menu, to get a human or a superior to decide.** Tower cannot see those native interactive menus, so under unattended mode they deadlock the terminal forever. Whenever you need someone to make a call you can't make yourself — or before any dangerous / irreversible operation — climb this ladder instead. Escalation only ever goes **upward** (child → parent → human), never back down.",
      "- **You were derived by a parent task** (your task description carries a `## 来源` section that says `父任务派生`): do **not** message a human directly. State the blocker and the concrete options as a plain-text final message, then end your turn normally — Tower automatically wakes the parent task to decide, and the parent injects its decision straight back into your terminal (you resume from there). If the parent can't decide either, escalating further up to a human is the **parent's** job, not yours.",
      "- **No parent + unattended mode** (the user entered `tower-goal`): you own the escalation — use `ask_human` (needs a reply → then stop and wait) or `push_to_human`, exactly as the Unattended section below describes.",
      "- **No parent + attended (not unattended)**: don't send any external message — just ask the question in plain text in this terminal and wait for the person to answer here.",
      "- **Anti-loop**: when you are the parent reviewing a stuck child, either make the call yourself or escalate further up — **never bounce the same question straight back down** unanswered, or the child just spins in place.",
      "",
  ...
  "task.workbenchDirective": {
    defaultValue: [
      ...
      "",
      "## Unattended mode / communicating with humans",
      "- **Never wait on the CLI's native `AskUserQuestion` prompt or a plan / option menu to get a human answer** — Tower cannot see those native menus and an unattended run deadlocks on them. When you need a human decision, use the tools below instead. (As the workbench you have no parent to escalate to, so a human is your next rung up.)",
      "- The user activating the `tower-goal` skill enters unattended autonomous long-running mode (activation grants authorization); in that mode use the `tower-ask` skill to communicate with humans: first call `list_notify_targets` to get the real channels. The active gateway only supports Hermes / OpenClaw; calling `push_to_human` sends externally first and then automatically records via `ask_human`/`notify_human`. A work-group message must pass the user-specified group/person as `to`; unattended mode may fall back to the configured owner/home. Don't park before you've confirmed the send succeeded.",
