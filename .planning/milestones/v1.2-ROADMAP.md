...
- 🚧 **v1.2 Extensions & Slim Distribution** — Phases 71-73 (planning)

## Phases

### 🚧 v1.2 Extensions & Slim Distribution (In Progress)

#### Phase 71: Extension Detection & Wiring

**Goal:** Build the unified `Extension` abstraction (registry + types + actions + client hook) that backs both rg and Monaco, replacing the current ad-hoc `installRg` action and `copy-monaco.js` postinstall script. Detail page conditionally renders 搜索 / 文件 tabs based on extension status.

**Requirements:** EXT-01..09

**Success Criteria:**
- `src/lib/extensions/{types,registry,client,context}.ts` + `definitions/{ripgrep,monaco}.ts` exist
- `src/actions/extension-actions.ts` with `checkExtension` / `installExtension` / `uninstallExtension` / `listAllExtensionStatus`
- rg install via `pnpm add @vscode/ripgrep` (no sudo); detection dual-track (package binary first, then `which rg`)
- Monaco install: `pnpm add monaco-editor` + run `scripts/copy-monaco.js`; detection checks both `node_modules/monaco-editor` AND `public/vs/loader.js`
- `package.json` postinstall no longer auto-runs `copy-monaco.js`
- Detail page hides 搜索 / 文件 tabs entirely when their extension is missing
- `code-search.tsx` uses `useExtension('rg')`; inline rg install panel removed
- Hot-reload — install/uninstall cycle does not require app restart

#### Phase 72: Extensions Settings Tab

**Goal:** Surface extension management in Settings — dedicated tab with one card per extension showing status (installed version + path, or not installed), Install / Uninstall / Reinstall / 访问官网 / 重新检测 buttons. Cards driven by the registry (zero-code addition for new extensions).

**Requirements:** SETTING-EXT-01..04

**Success Criteria:**
- Settings page has new "Extensions" tab in sidebar
- Cards laid out per `Extension` registry entry, no per-extension UI code
- Inline progress (spinner + "安装中…") on install/uninstall buttons
- Toast on success / failure with truncated error message

#### Phase 73: Onboarding Integration

**Goal:** Onboarding wizard adds an "扩展启用" step. Both extensions default-checked. Unchecking shows hint pointing to Settings → Extensions. On submit, install in parallel; failures surface as deferred toast on first workspace load.

**Requirements:** ONBD-EXT-01..05

**Success Criteria:**
- Wizard step renders one row per extension (name + size + checkbox + 访问官网)
- Defaults to all checked
- Unchecking surfaces "可在 设置 → Extensions 中随时启用" inline hint
- Submit installs in parallel via `Promise.all`
- SystemConfig persists `onboarding.extensions.requested` and `…completed`
