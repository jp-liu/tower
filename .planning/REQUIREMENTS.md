...
- [x] **FORM-02**: User can import project via browse dialog, selected localPath is read-only (not editable)
- [x] **FORM-03**: User can migrate project with editable target path
- [x] **FORM-04**: Project description and task description textarea have max-height with overflow-y scroll (prevent dialog height overflow)
- [x] **FORM-05**: Git cloneDir setting shows warning text "请输入绝对路径，不支持 ~ 别名", backend rejects paths starting with ~

## UI Polish (界面优化)

- [x] **UI-01**: Assistant chat icon moved from language toggle area to right side of search box in header
...
- [x] **SEARCH-01**: Detail page left panel has two tabs: "文件树" (existing) and "搜索" (new)
...
- [x] **SEARCH-05**: Clicking a search result opens the file in Monaco editor at the matching line
