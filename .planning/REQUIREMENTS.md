...
- [ ] **WB-01**: 用户可从任务抽屉点击"查看详情"跳转到任务专属工作台页面
...
- [ ] **PV-01**: 项目支持"前端/后端"类型区分，创建时默认前端，可选后端
- [ ] **PV-02**: 前端项目工作台有预览面板，包含地址栏输入本地 URL + iframe 嵌入显示
- [ ] **PV-03**: 用户可配置项目预览启动命令，点击"运行"按钮启动 dev server
- [ ] **PV-04**: 预览面板提供"在终端打开"按钮，在本地终端 app 中打开 worktree 目录
- [ ] **PV-05**: 用户可在设置中配置默认终端应用（iTerm2/Terminal.app/Warp 等）
