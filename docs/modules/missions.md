...
- **启动任务搜索弹窗**：点击「启动任务」直接打开搜索弹窗，Fuse 模糊搜索任务，或按「工作区 → 项目」浏览，并展示最近任务列表（一次加载至多 100 条最近任务，搜索结果上限 30 条）。弹窗打开时**不自动聚焦搜索框**（Base UI `initialFocus`）——需按一次 `Tab` 才聚焦，避免浏览器的历史下拉在弹窗上方遮挡干扰
