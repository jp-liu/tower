...
- **版本归纳**：任务可归属到项目版本（Version Timeline），按版本组织与回顾；未指定版本的任务进入 backlog。MCP 侧通过 `list_versions` 查询可用版本，`create_task` / `update_task` 的 `versionId` 指定归属；`update_task` 传 `null` 或空串可将任务移回 backlog
