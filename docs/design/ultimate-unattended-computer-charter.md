...
> 状态：定稿；首个模块化与恢复增量已进入实现验证
> 日期：2026-08-01
> 基线：Claude / Superpowers 评审 -> 架构图 -> Technical Spec -> 小步纵向实现

本文记录 Tower、o-tower、OpenClaw 与外部 Operator 在“终极无人值守电脑”目标下的长期边界。
前两步由讨论确认，其余边界由当前候选稿统一收敛。本文只确定系统职责、跨系统契约语义和可靠性
原则，不提前决定具体 API、数据库迁移或代码任务。具体实现状态、验证证据和未完成项以架构图解与
Technical Spec 为准。
...
每个无人值守 Goal 至少具备以下上限：

- 总运行时长；
- provider 回合数；
- 子任务总数和并发数；
- 连续失败或无进展次数；
- 外部 Capability Job 次数；
- 可获取时的 token / 金额预算。

达到上限后的统一动作是保存状态、转 `BLOCKED`、向 OWNER 说明已完成内容和继续所需授权；不能静默
杀死后伪装完成，也不能无限重试。现有 `BudgetGuard` 只有在真正接入执行与唤醒路径后才算交付完成。
...
完成通知。需要 OWNER 最终验收时可以进入等待，但不能继续消耗执行资源。
...
架构原则由评审记录、静态检查或依赖审计验收；具备可执行行为的断言才成为 CI 契约测试或 E2E 门禁。
本机真实 Operator E2E 可以是发布前环境验收，但不能成为普通 npm 安装或无本机扩展 CI 的前提。

## 18. 交付顺序与当前进度

章程阶段采用过以下顺序，架构与图面评审现已完成：
...
## 20. Claude / Superpowers 评审关注点

1. Tower、OpenClaw、Capability Port、Operator 的状态所有权是否仍有重叠？
2. 非 LLM Capability Port 是否真正消除了 o-tower 二次推理，而没有形成第二个 Gateway？
3. 一个通用 envelope 是否足够，还是某类 capability 确有独立契约需求？
4. `SIDE_EFFECT_UNKNOWN`、取消和 timeout 规则是否覆盖真实外部系统的不确定性？
5. 无人值守时由谁签发 R2 / R3 限域预授权，如何绑定未来请求、限制目标/次数/有效期并防止任务 Agent
   自我授权或重放？这是 Technical Spec 必须解决的核心开放问题；未解决或运行时无有效授权时必须
   `BLOCKED` 并询问 OWNER。
6. Goal 唤醒和看门狗是否依赖持久事实，而不是终端静默或易失内存？
7. OpenClaw 持久化 Job、Tower 保存关联的分工是否满足恢复，又避免影子状态机？
8. 哪些部分属于必要复杂度，哪些仍可在首个纵向闭环中推迟？
9. 当前 `tower-bridge`、o-tower profile、`push_to_human` 和 Operator 路径如何最小迁移？
10. 产品默认能力与本机扩展是否在发布、升级、卸载和文档上彻底分离？
