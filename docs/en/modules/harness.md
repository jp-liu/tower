...
- **No detection of Claude CLI native option menus**: if a child task gets stuck on Claude CLI's native blocking interactions (`AskUserQuestion` / plan option menu), Tower **cannot detect it** and an unattended run will hang. Proposed direction: steer child tasks to always use `ask_human` whenever a human decision is needed and disable native blocking interactions (Option A, **undecided / not implemented**).
