...
7. **Only user-supplied attachments go in `references`** on `create_task` (pasted images, screenshots, documents, or bridge media paths; base64-only → `manage_assets upload` first, then pass the returned path). Never put source files discovered while searching a repository in `references`; list those as repository-relative paths under `## 参考`. For OpenClaw/Hermes bridge messages, if the prompt/tool context contains a local media path (for example under `.openclaw/.../media/inbound`, `.openclaw/.../attachments`, or `.hermes/...`), pass that path in `references`; do not only summarize the picture in `description`.
...
10. **Task binding is read-only until an action is chosen.** Platform replies containing or quoting `[[tower:task=...]]` first use `resolve_gateway_task_context`. A status/result question uses read-only tools. An external-system request is delegated in the Gateway with the returned context and does not change Tower. When an OPEN ask requests approval for external Operator work, execute that one approved operation in OpenClaw first, then use `reply_to_ask` with the validated result; never inject a bare approval before execution. Other OPEN asks use `reply_to_ask` directly. Only an explicit continue/fix/rerun request uses OWNER-only `continue_bound_task`. `relay_channel_reply` is compatibility-only and no longer resumes ordinary task replies.
11. **External send/receive** follows [references/unattended-messaging.md](references/unattended-messaging.md) through `tower-bridge`: unattended OWNER messages use one authorized CapabilityRequest; explicit work-recipient messages use `push_to_human`; every outbound carries `[[tower:task=<taskId>]]`.
...
13. **Tower is not the global message bus.** Ordinary Q&A and external operator requests stay in the gateway and never call `route_gateway_message`. Tower-related non-reply messages use that tool; Tower-delivery replies follow Contract 10. Never guess an ambiguous project, never say project work created a task while it is only queued, and use `complete_gateway_discussion` / `confirm_gateway_task_created` / `complete_gateway_work` for their corresponding durable replies.
14. **A durable Workbench batch requires explicit acknowledgement.** When the prompt contains `[Tower durable batch: ...]`, immediately call `ack_workbench_batch` with that batch id before doing the work. Until every item is handled or durably delegated, call `heartbeat_workbench_batch` with the same lease token every two minutes; do not wait for the five-minute lease to expire. Then call `resolve_workbench_batch`. A replay of the same batch id is recovery, not a new request; inspect existing links and continue idempotently.
...
- Repository source paths go only in `参考`, using repository-relative paths; never upload searched source files as assets. User-provided attachments go in `references` and may also be described in `参考`. A screenshot summary is not a substitute for the original image; include its local attachment path in `references` whenever one exists.
