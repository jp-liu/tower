...
14. **A durable Workbench batch requires explicit acknowledgement.** When the prompt contains `[Tower durable batch: ...]`, immediately call `ack_workbench_batch` with that batch id before doing the work. After every item is handled or durably delegated, call `resolve_workbench_batch`. A replay of the same batch id is recovery, not a new request; inspect existing links and continue idempotently.
